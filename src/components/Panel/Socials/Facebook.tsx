...
interface FacebookID extends Facebook {
  id: number
}
...
const Facebook = () => {
  ...
  const {data, loading, error} = useApi("/socials/Facebook")
  const [newData, setNewData] = useState<Facebook[]>(data)
  const [debounceData] = useDebounce(newData, 800)
  const updateStatus = useUpdateApi("/socials/updateFacebook", debounceData, data)

  const [currentSocial, setCurrentSocial] = useState<FacebookID | undefined>(undefined)
  ...
  return <>
    <Box textAlign="center">
      ...
      <Text>Las opciones te permiten modificar las alertas sobre usuarios de Facebook</Text>
    </Box>
    <br/>
